...
<xsl:stylesheet xmlns:xs="http://www.w3.org/2001/XMLSchema"
  ...
  <xsl:template match="*[self::bibl | self::biblFull]//ref[matches(., '^(DOI|https://doi.org/)', 'i')]" mode="tei2bits">
    <pub-id>
  ...
  <xsl:template match="forename" mode="tei2bits">
    <given-names>
      <xsl:apply-templates select="@* except @rend" mode="#current"/>
      <xsl:value-of select="normalize-space(replace(string-join(text()), '\p{Zs}(de|von|van|v\.)', '', 'i'))"/>
    </given-names>
    <xsl:if test="matches(string-join(text()), '\p{Zs}(de|von|van|v\.)', 'i')">
      <suffix content-type="particle">
        <xsl:value-of select="normalize-space(replace(string-join(text()), '^.+?\p{Zs}(de|von|van|v\.)(\p{Zs}(de|von|van|v\.))*([\P{L}].*$|$)', '$1$2', 'i'))"/>
      </suffix>
    </xsl:if>
